...
- [bin/init.ts](../../bin/init.ts) — `PACKAGE_ROOT`·`REQ_SCRIPTS`·`REQ_DEV_DEPS`·`assertGitWorkTree`에 `export` 추가(동작 불변) + `printHelp`에 `uninstall` 1줄.
...
8. **ticketRoot 존중**: `req.config.json`의 `ticketRoot`가 `docs/req-tickets`인 repo에서, 증거 디렉터리를 하드코딩 `workflow/`가 아니라 설정값에서 인식한다.
9. **npx 캐시 섹션**: 출력에 `_npx` 제거 명령과 **`npm cache clean --force`가 제거 명령이 아니라는 경고**가 포함된다.
10. **package.json 안내 형태**: 출력에 `git checkout HEAD -- package.json`이 있고 인덱스 기준 `git checkout -- package.json`은 **없다**.
...
- 대상 repo에 추가되는 것: `scripts/req/`, `<ticketRoot>/*.schema.json`, `req.config.json`, `AGENTS.md`, `package.json`의 `req:*` scripts + `ajv`/`cross-spawn`/`tsx` devDependencies.
