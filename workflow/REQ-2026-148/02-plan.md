# REQ-2026-148 계획 — phase 분해 (DEC-WF-027 §9.0)
...
**`req-confirm --method`**)이 예약값을 거부.

Exit:
- 🔴 **도구가 낸 원문 명령을 그대로 실행하면 거부**된다 — `--resolve replace` · `req:close --abandon` ·
  **`req:confirm --method`**(설계 r07 P1: `req:next` 가 `--method "<승인 문장>"` 을 낸다) 전부.
  정규화 후 비교라 `"  승인  문장 "` 도 걸린다.
- 🔴 **안내를 내는 표면과 값을 받는 표면의 짝이 맞는지** 소스 가드로 고정한다 — 한쪽만 늘면 고리가
  다시 열린다(이 REQ 가 r01·r07 에서 두 번 그렇게 놓쳤다).
...
- 계약 스위트: `npx vitest run tests/unit/placeholders.test.ts tests/unit/shell-safe.test.ts tests/unit/next-policy-guidance.test.ts tests/unit/nonconvergence.test.ts tests/unit/req-close.test.ts tests/unit/req-confirm.test.ts tests/unit/dispatch.test.ts`
- Codex 승인.
