...
- 🔴 **DEC-1a — 미커밋 `.gitignore` 재현 e2e 2건**(설계 r01·r02 P1). 둘 다 안내 전체를 실행하면
  **끝까지 성공**해야 한다:
  ① **루트**: `.gitignore`(미커밋, `node_modules/`) + `node_modules/`.
  ② **중첩**: 루트는 깨끗하고 `packages/app/.gitignore`(미커밋) + `packages/app/node_modules/`.
  (지금은 stash 가 ignore 규칙을 되돌려 `?? …node_modules/` 가 드러나고 `req:new` 가 거부된다 —
  ①은 실측 재현 완료.)
- 🔴 **조건부 줄**: 미커밋 `.gitignore` 가 없으면 그 줄을 **내지 않는다** — 내면 "커밋할 것이 없다"로
  실패해 "안내가 순서대로 성공한다"는 계약을 스스로 깬다. 양쪽 분기 모두 테스트한다.
- 🔴 **목록은 실제 경로**다 — glob(`**/.gitignore`) 을 내지 않는다(고정 문자열 부재 검사).
  여러 개면 한 줄에 모두 나온다.
- 🔴 **셸 안전하지 않은 경로**(공백 등)가 있으면 그 갈래의 **명령을 통째로 내지 않는다** —
  반쪽 명령열 금지(REQ-2026-149 계약 무회귀). 회귀 테스트로 고정.
- 🔴 `terminalReentryProblem` 은 **여전히 순수**하다 — 사실(`dirtyGitignores: string[]`)을 인자로 받는다.
  호출부가 그 사실을 실제로 넘기는지 **소스 가드**로 고정한다(배선 끊김 방지).
...
Exit:
- 🔴 **`validateManifest` 회귀**: `null` · 숫자 · 빈 문자열 · 63자리 · 65자리 · 비-hex(`g` 포함)
  각각이 **거부**된다. **키 부재는 통과**한다(레거시 무회귀).
  🔴 **대문자 hex 는 통과한다** — 기존 `SHA256_RE`(`/^[0-9a-f]{64}$/i`)가 대소문자를 모두 받고
  `response_sha256`·`design_hash`·`phase_design_ref` 가 이미 그 계약이다. 새 키만 다르게 두면
  같은 저장소 안에 두 규칙이 생긴다(설계 r01·r03 관찰). 회귀 테스트로 **통과**를 고정한다.
