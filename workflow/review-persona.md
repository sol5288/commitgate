...
- **phase 분해**: `02-plan.md`의 각 phase가 **책임 계약·입력·산출물·선행 phase·독립 검증 명령**을 선언하는지
- **독립성**: 선언된 선행 phase만 충족되면 각 phase가 **독립 커밋·독립 리뷰**되는지
- **숨은 결합**: 후속 phase가 있어야만 현재 phase의 인수 기준이 성립하지는 않는지

**🔴 분해 관점은 탐색의 하한이지 P1 기준의 확대가 아니다.** 분해 결함이 `findings`가 되려면 위 "P1 정의"의 3요소(카테고리·정상 경로·증거)를 **그대로** 만족해야 한다. 분해가 더 나을 수 있다는 판단, 경계 취향, 더 잘게 쪼갤 수 있다는 제안은 전부 `observations`다.

분해를 차단 사유로 쓰려면 **그 분해로 진행했을 때 정상 경로에서 무엇이 깨지는지**를 제시해야 한다 — 예를 들어 phase N의 인수 기준이 phase N+1 없이는 검증 불가라 승인 자체가 무의미해지는 경우. 그런 재현 경로 없이 분해를 지적하는 것은 P1이 아니다. **한 티켓이 감당 못 할 만큼 결합돼 있다는 판단은 그 자체로는 차단이 아니라 `observations`이며, 종결·재분할은 사람의 결정이다.**
