...
**지원됨 → 후속 REQ 우선화 = 검증 도구 위임 / 오라클 품질 개선.**

- 장기화의 지배 원인은 **③b(빠진/약한 검증 oracle·명세 갭의 점진적 발견)** 21/32 — 대부분 "near-e2e oracle 부재"·"grep 약함"·"byte-identity 미고정"·"CommonMark 미준수" 형태로, 리뷰어가 매 라운드 **다른** 검증 공백을 지적한다.
- 🔴 **② 닫힌 finding 재론 = 0/32.** 이는 resume 질문의 **실증적 종결**이다: 재론이 없으므로 세션 유지(resume)나 Review Context Ledger는 도울 대상이 없다 — 게이트 무-resume 확정이 데이터로 뒷받침된다.
- **REQ-032**(유일 온전 6R 터미널 비수렴) = **5/5 ③b** — freeze 가설(터미널 비수렴 = 재론 아니라 oracle 갭 점진 발견)을 정확히 확증. [[commitgate-readme-restructure]] REQ-041→042(손수 oracle→`remark-validate-links` 위임) 교훈과 정합.

**후속 REQ 후보**: 설계·계획 단계의 **검증 oracle을 도구/패턴에 위임** — ①near-e2e/main-통합 oracle 표준 패턴(reviewer 주입 seam·attempt-보존·전송 prompt byte-identity), ②CommonMark 등 복잡 명세는 파서 라이브러리 위임(040 펜스 = REQ-041형), ③touchpoint별 exact assertion(grep 금지). Ledger·resume은 **불필요**(② 부재).

## 한계 (🔴 선택편향 — freeze §7)

- 소실 3(026·041·044-design) 제외. 이 중 **026·041 = 터미널 비수렴**(가장 심한 사례)인데 아카이브 부재. 041은 손수 링크-oracle 7R 비수렴([[commitgate-readme-restructure]])으로 **③b가 명백**한 사례 → 소실분은 ③b를 **과소**표집한다(편향이 결론을 강화하지 반증하지 않음).
