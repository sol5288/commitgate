...
### D5. 페르소나 = 사용자 편집 불가침 (R5) — 부재만 복원, 차이는 report-only

**커스터마이즈의 정식 탈출구는 `reviewPersonaPath`를 사용자 파일로 지정하는 것**이다. 기본 경로(`workflow/review-persona.md`)는
kit 관리 위치이나, **내용이 다르면 사용자가 그 위치를 직접 고쳤을 수 있으므로 절대 덮지 않는다**(Codex design-r02 P1).

```
--persona 없음                          → 페르소나 미접촉 (plan은 drift를 advisory로만 표시)
--persona + reviewPersonaPath===null    → 'unmanaged (disabled)', 미접촉
--persona + custom 경로(≠기본, 절대경로) → 'unmanaged (custom path)', 미접촉
--persona + 기본 경로 + 부재(st===null) → apply: shipped 복사 (잃을 것 없음 — 부재면 loadReviewPersona가 이미
                                           fail-closed로 리뷰를 멈추므로 복원이 순이득)
--persona + 기본 경로 + sha 동일         → in-sync, skip
--persona + 기본 경로 + 다름             → 🔴 report-only, **미접촉**(덮지 않음). manifest 없이 stale-kit과 사용자
                                           편집을 구별할 수 없으므로 절대 파괴하지 않는다. plan/apply 모두
                                           "기본 persona가 shipped와 다름 — 수동 확인(사용자 편집이면 유지, stale면
                                           직접 교체 또는 STR-06 3-way 대기)"만 출력.
```

- **`--persona`의 유일한 쓰기 = 부재 복원**이다. 내용이 다른 파일은 `--persona`가 있어도 **결코 덮지 않는다** — R5의
  "사용자 수정본을 절대 덮지 않는다"를 코드 경로로 보장(design-r02 P1). 그래서 sync는 **`init --force`보다 persona에
  대해 의도적으로 더 보수적**이다(init --force는 KIT_COPY_RELPATHS를 무조건 덮지만, sync는 안 덮는다).
- stale(미편집)인데 다른 경우도 보존되어 refresh가 안 되지만, 그 비용은 **리뷰 라운드 품질 저하**일 뿐 게이트 결정이
  아니다(게이트 결정 자산 machine.schema.json은 D4가 --force 축으로 확실히 갱신). 자동 3-way 복구는 STR-06 후속.
- **"기본 경로" 판정도 정규화 절대경로 기준**(D6 P1과 동일 이유): `cfg.reviewPersonaPathAbs === resolve(cfg.root, DEFAULT_REVIEW_PERSONA_RELPATH)`.
  문자열 비교면 `./workflow/review-persona.md` 동치 경로를 custom으로 오판한다.
- 결과: **custom·null·기본경로-편집 페르소나 전부 어떤 경우에도 미훼손.** sync가 persona에 하는 파괴적 쓰기는 0건이다.
...
phase-1(헬퍼 export) → phase-2(skew 해결 전체 = sync + D20 + 문서) **2단계**.

> ⚠️ **phase 재분해(design r04)**: 원래 4-phase(sync / D20 / 문서 분리)였으나, phase 리뷰가 각 얇은 slice를
> "REQ 전체 미완"으로 판정하는 **diff-scoped over-reach**가 발생했다(sync-only diff를 R7·R1 미충족으로 차단 —
> persona의 "phase 리뷰는 해당 phase 인수 기준만" 규칙 위반이지만 리뷰어가 00-requirement의 R을 요구 위반으로 읽음).
> skew 해결은 **감지(D20)+복구(sync)+문서가 하나의 일관 단위**이므로 phase-2로 병합한다. phase-2 id는 유지(리뷰
> 시리즈 연속성). 병합 phase는 코드 변경 >8파일이라 D18 granularity WARN(advisory)이 예상되나 FAIL 아님.

| phase | 범위 | 변경 파일 |
|---|---|---|
| `phase-1-export-helpers` | R6 — `assertConfinedDest`·`statWritableDest`·`sha256File`에 `export` 추가. **동작 무변경**. export 존재 assert *(완료·승인)* | `bin/init.ts`·`tests/unit/init.test.ts` |
| `phase-2-sync-verb` (=skew 해결 전체) | D1~D6 전부(R1~R9) — ① sync verb(동기 runCli·plan/apply·`--dir`·`--persona`·packageRoot 가드) + dispatch·help ② doctor **D20**(content-hash WARN·DoctorInputs 필드·main() sha/version) ③ README(ko/en) 거짓 주장 교정 + "업그레이드(0.x)" 절 ④ CHANGELOG·SSOT(G-10/STR-06 부분 해결) ⑤ 회귀망 2개 | `bin/sync.ts`·`bin/dispatch.mjs`·`bin/commitgate.mjs`·`scripts/req/req-doctor.ts`·`README.md`·`README.en.md`·`CHANGELOG.md`·`docs/ssot-design/*`·`tests/unit/{sync,dispatch,req-doctor,init,req-review-codex}.test.ts` |

## 변경 파일

- `bin/init.ts` — 세 헬퍼 `export`(동작 무변경, **phase-1 완료**)
- `bin/sync.ts` — 신규 verb (phase-2)
- `bin/dispatch.mjs` — `sync` VERB_MODULES 등록 (phase-2)
- `bin/commitgate.mjs` — help docstring (phase-2)
- `scripts/req/req-doctor.ts` — D20 + DoctorInputs 필드 + main() 계산 (phase-2)
- `tests/unit/{sync,dispatch,req-doctor,init,req-review-codex}.test.ts` — 신규·갱신 (phase-2)
- 문서: `README.md`·`README.en.md`·`CHANGELOG.md`·`docs/ssot-design/gaps-and-decisions.md`·`docs/ssot-design/14-product-strategy-and-roadmap.md` (phase-2)
...
- **fail-closed 방향**: sync 기본 dry-run(쓰기 0건). persona에 대한 sync의 유일한 쓰기는 **부재 복원**뿐 — custom·null·기본경로-편집
  전부 미훼손이고 내용이 다르면 report-only(design-r02 P1). D20은 sha 계산 불가·dogfood·custom·동일에서 OK(오탐으로 게이트 막지 않음), 상이에서만 WARN(게이트 안 막음).
