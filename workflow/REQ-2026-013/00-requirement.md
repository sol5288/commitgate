...
- codex exit≠0 오류는 **allowlist**로만 표면화한다 — 허용 이벤트(`turn.failed`/`error`/`stream_error`)의 허용 문자열 필드만, byte 상한(Buffer.byteLength)으로. 미지 이벤트·중첩 payload·파싱 실패는 **폐기**. 허용 이벤트가 없으면 **raw stdout을 싣지 않고** exit code + stderr + 생략 표식만(빈 오류는 없애되 비밀은 안 샌다).
- 재리뷰가 **항상 새 스레드**다(`codex_thread_id`가 있어도 resume 안 함). `--fresh-thread`의 blocked-마커 회복은 보존. **`state.last_review`가 직전 same-target NEEDS_FIX일 때만** 그 findings가 bounded로 프롬프트에 주입되고, **승인 후에는 재주입되지 않는다**(승인 경계 — 해소된 결함 재주입 방지).
