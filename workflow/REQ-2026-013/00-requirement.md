...
- codex exit≠0 오류는 **codex 경계(`defaultCodexRunner`)에서** **allowlist**로만 표면화한다(범용 `safeSpawnSync`는 stdout 파싱 안 함 — 비-codex 명령 유출 방지). 허용 이벤트(`turn.failed`/`error`/`stream_error`)의 허용 문자열 필드만, **총량 상한**(최대 N 이벤트 + 총 UTF-8 byte ≤ 8KiB, 초과 시 단일 생략 표식). 미지·중첩·파싱 실패는 **폐기**. 허용 이벤트 0개면 raw stdout 미포함. stderr는 **best-effort 비밀 마스킹(redaction) + byte-bounded**(byte 상한은 기밀성 보장이 아님).
