...
- codex exit≠0 오류는 **codex 경계(`defaultCodexRunner`)에서** **allowlist**로만 표면화한다(범용 `safeSpawnSync`는 stdout 파싱 안 함 — 비-codex 명령 유출 방지). 허용 이벤트(`turn.failed`/`error`/`stream_error`)의 허용 문자열 필드만, **총량 상한**(최대 N 이벤트 + 총 UTF-8 byte ≤ 8KiB, 초과 시 단일 생략 표식). 미지·중첩·파싱 실패는 **폐기**. 허용 이벤트 0개면 raw stdout 미포함. stderr도 byte-bounded.
- 재리뷰가 **항상 새 스레드**다(`codex_thread_id`가 있어도 resume 안 함). `--fresh-thread`의 blocked-마커 회복은 보존. 검증된 findings의 **bounded 스냅샷을 `state.last_review`에 원자적으로 기록**(가변 `codex-response.json` 아님 — desync 방지). 재리뷰는 `last_review`가 직전 same-target NEEDS_FIX일 때만 그 스냅샷을 주입하고, **승인 후에는 재주입되지 않는다**(승인 경계).
