...
- `reviewReasoningEffort`: `{ type:['string','null'], enum:['minimal','low','medium','high','xhigh', null] }`. **null을 enum에 포함**해야 탈출구 성립(JSON Schema enum은 타입 무관 전체 적용 — null이 없으면 `{effort:null}` 거부).
...
**D4. `reviewReasoningEffort`는 Codex 공식 enum**(`minimal|low|medium|high|xhigh`+null, config-reference WebFetch 확인). config-load에서 오타 거부. `reviewModel`은 값 공간이 열려 slug 패턴으로 최소 방어.
...
- **override 존중 검증**: 자기-리뷰 성공은 "override 적용"과 "무시하고 ultra 상속"을 구분 못 하고, prior thread_id로 일반 실행은 resume 경로다. 그래서 **bogus-model live 검증**(수동/smoke): `-c model="__bogus__"` → codex `item.completed{type:error}`/`turn.failed`가 "Model … not found"(도달·해석 증명). **exec·resume 양쪽**(exec는 R5 캡처로 확인, resume는 Phase 1에서 prior thread_id로).
...
**D9. 테스트 이음새.** 실 어댑터 + 주입 `CodexRunner`(args 캡처: `-c model=`/`-c effort=` exec·resume·null이면 없음) · `FakeReviewerAdapter.requests`(DTO 전파) · `loadConfig`(null 보존·enum 거부·패턴 거부·두 축 동기화) · stateless(항상 fresh·`--fresh-thread` marker-clear 배선·findings 주입·read 검증·G2 불변·교차-대상 미전달) · bogus-model live(수동/smoke).

## Phase별 구현

### Phase 1 — 모델·추론강도 고정 (`phase-1-model-effort-pin`)
config `reviewModel`(패턴)·`reviewReasoningEffort`(enum+null)(D1·D4, null 보존, 두 스키마 축) + `ReviewRequest`·`callReviewer`·`review()` `-c` 주입(D2·D2-1) + 테스트(args 캡처·null 보존·enum/패턴 거부·DTO). bogus-model live 검증(exec·resume, D7). 문서 제외(Phase 3).
...
- **미검증.** codex `-c model_reasoning_effort=` 존중은 phase에서 bogus-model live 확인.
