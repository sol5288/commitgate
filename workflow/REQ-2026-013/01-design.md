...
- **stderr(및 추출 메시지)는 redaction + byte-bounded(≤4KiB)** — byte 상한은 기밀성 보장이 아니므로(R4), **best-effort 비밀 마스킹**을 적용한다. **정규식은 JavaScript 문법**(R6 — `(?i)` 인라인 플래그는 JS에서 `SyntaxError`; `g`+`i` 플래그 사용)이고, **Bearer 토큰은 공백 뒤까지 소비**한다(R6 — `\S+`가 `Authorization: Bearer <jwt>`에서 `Bearer`까지만 잡아 토큰이 새던 버그):
  ...
  ```
  **적용 대상(R11)**: 추출 오류 메시지는 `error.message` 등을 **JSON.parse로 한 겹 디코드한 값**(실제 따옴표)에 redaction을 건다 — codex의 `error.message`는 JSON-인코딩 문자열이라(R5 캡처: `"{\"...\":...}"`) 디코드하면 `{"token":"x"}`가 되어 위 JSON 규칙이 잡는다. stderr(자유형식)도 같은 규칙. 적용 후 ≤4KiB 절단, `[redacted]` 표식. **best-effort(모든 비밀 형식 보장 아님 — 예: stderr의 이중-이스케이프 중첩 JSON)** — 한계 문서화, 그래서 raw dump를 안 하는 것이 1차 방어. node로 규칙 컴파일·마스킹 실측 검증. 회귀: `token=abc`·`Authorization: Bearer x.y.z`(뒤 토큰까지)·**`{"token":"abc"}`·`{"api_key":"sk-…"}`(JSON 형태)**·`sk-…` 각각 마스킹 후 byte 절단.
