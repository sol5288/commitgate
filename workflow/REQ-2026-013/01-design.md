...
- **`safeSpawnSync`는 범용이다**(git·npm·pnpm도 호출, design R3) — 여기서 **stdout JSONL을 파싱하지 않는다**(그건 codex 경계 지식, D6). 실패 시 원시 필드 + **분류 태그**(`{kind:'timeout'|'buffer-overflow'|'exit', status, signal, stdout, stderr}`)를 담은 오류를 throw. **범용 메시지 = exit + `redactSecrets(stderr)`(byte-bounded)** — redaction은 **codex-특화가 아니라 범용 비밀-마스킹**(순수 regex 유틸, D6의 `REDACTIONS`)이므로 여기서 적용한다(감사 개선): npm/pnpm 실패도 stderr 진단을 **유지하면서** 토큰을 마스킹한다(R8의 "exit만"은 npm/pnpm 진단을 잃었음). stdout은 미포함(임의 명령의 stdout은 데이터/비밀이고 allowlist가 없다 — 원시 fields로만 보관). JSONL **추출**은 codex 경계에서만(D6).
- **분류 보존 계약(R5·R8, D5↔D6 경계)**: stderr는 이미 범용 redaction됐다(위). `defaultCodexRunner`는 **분류(kind·문구)를 보존**하고 — `kind==='exit'`일 때만 stdout allowlist **추출**(redacted, D6)을 덧붙이고, timeout/overflow는 추출 없이(stdout 무관) redacted-stderr 메시지를 그대로 쓴다. 그래서 세 kind가 구분되면서 어느 kind든 stderr가 마스킹된다. `defaultCodexRunner` 경유 회귀로 (a) 분류 보존 (b) 모든 kind stderr redaction (c) exit만 JSONL 추출을 고정(D11).
...
- **stderr(및 추출 메시지)는 redaction + byte-bounded(≤4KiB)** — byte 상한은 기밀성 보장이 아니므로(R4), **best-effort 비밀 마스킹**을 적용한다. **정규식은 JavaScript 문법**(R6 — `(?i)` 인라인 플래그는 JS에서 `SyntaxError`; `g`+`i` 플래그 사용)이고, **Bearer 토큰은 공백 뒤까지 소비**한다(R6 — `\S+`가 `Authorization: Bearer <jwt>`에서 `Bearer`까지만 잡아 토큰이 새던 버그):
  ...
  ```
  **적용 위치·대상**: `REDACTIONS`는 순수 유틸이라 **두 곳**에서 쓴다 — (a) `safeSpawnSync`가 **모든 명령의 stderr**에 범용 적용(D5, npm/pnpm 진단 유지+마스킹), (b) codex 경계가 **추출 오류 메시지**에 적용. 추출 메시지는 `error.message`를 **JSON.parse로 한 겹 디코드한 값**(실제 따옴표)에 건다 — codex의 `error.message`는 JSON-인코딩 문자열이라(R5 캡처) 디코드하면 `{"token":"x"}`가 되어 JSON 규칙이 잡는다. 적용 후 ≤4KiB 절단, `[redacted]` 표식. **best-effort(모든 비밀 형식 보장 아님 — 예: stderr의 이중-이스케이프 중첩 JSON)** — 한계 문서화, raw dump를 안 하는 것이 1차 방어. **node로 규칙 컴파일·마스킹 실측 검증(감사)**. 회귀: `token=abc`·`Authorization: Bearer x.y.z`(뒤 토큰까지)·`{"token":"abc"}`·`{"api_key":"sk-…"}`(JSON)·`sk-…`·**`https://user:pass@host`(URL basic-auth)** 각각 마스킹 · `https://registry/pkg`(auth 없음) 무변(오탐 없음) 후 byte 절단.
