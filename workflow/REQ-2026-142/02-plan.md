...
Phase 1 의 계약 스위트 예시에 적힌 **`tests/unit/evidence.test.ts` 는 존재하지 않는 파일**이다.
실제 파일명은 **`tests/unit/evidence-module.test.ts`** 다.

🔴 **이것이 위험한 이유**: `vitest run <경로>` 는 매칭되는 파일이 하나도 없어도 **exit 0** 으로 끝난다.
그 줄을 그대로 복사해 실행한 사람은 "계약 스위트 통과"를 보지만 실제로는 **그 파일이 한 줄도 돌지 않았다**.
없는 경로를 적어 두면 초록이 거짓말을 한다.

(REQ-2026-142 진행 시에는 실제 파일로 실행했고 526 테스트가 통과했다 — 문서의 예시 줄만 틀렸다.)
