...
| 안내를 내는 곳 | verb | 인자 | 그 값이 여는 것 |
|---|---|---|---|
| hardCap 보고 | `req:review-exception --resolve replace` | `--reason` · `--confirm` | 대체 REQ 개설 |
| hardCap 보고 | `req:close --abandon` | `--reason` · `--confirm` | 티켓 종결 |
| hardCap 보고 | `req:review-exception --close-stale` | `--reason` | 회차 폐기 |
| `req:next` | `req:confirm` | `--method` | HIGH 확인 |
| **`req:next`** | **`req:delegate`** | **`--sentence`** | 🔴 **main 병합 권한** |

🔴 **마지막 줄이 가장 중대하다**(설계 r01 P1). `stopGate:"auto"` 에서 `req:next` 는
`req:delegate … --sentence "사람이 말한 승인 문장" --run` 을 낸다. 그 원문을 실행하면 도구가 만든
범용 문자열로 **local_merge 위임이 발급되고**, 통합 게이트가 그것을 소비해 **사람 확인 없이 병합**한다.
REQ-2026-140 이 세운 "임의의 대화 문장으로 main 병합 권한이 생기면 안 된다"를 도구가 스스로 어긴다.

예약 목록에는 도구가 실제로 내는 문자열을 **그대로** 넣는다: `"승인 문장"` · `"<승인 문장>"` ·
`"사람이 말한 승인 문장"` · `"왜 대체하는가"` · `"왜 버리는가"`.

🔴 **표를 손으로 유지하지 않는다**: 안내를 내는 모든 코드 경로가 등록부를 **참조해서** 렌더링하고,
값을 받는 모든 verb 가 같은 등록부로 거부한다. 소스 가드가 **양쪽 목록의 짝**을 고정한다 —
이 REQ 는 표면을 세 번(r01·r07·149-r01) 놓쳤고, 손 관리로는 또 놓친다.
...
- 🔴 **금지 목록이 아니라 허용 목록이다.** 셸마다 특수문자가 다르고(cmd.exe 는 큰따옴표 안에서도
  ...
  기본 거부**한다.
- 이 집합은 정상 branch 이름(`feat/req-2026-148-guidance-safety-and-attribution-successor`)·REQ id·티켓 경로·
  **series_id** 를 전부 담는다.
...
`scripts/req/req-review-exception.ts` · **`scripts/req/req-close.ts`**(148-r02 P1) ·
**`scripts/req/req-confirm.ts`**(148-r07 P1) · **`scripts/req/req-delegate.ts`**(149-r01 P1 — 이 값이
main 병합 권한을 연다) · `scripts/req/req-next.ts` · `scripts/req/review-codex.ts` ·
테스트 · `CHANGELOG.md`

🔴 **변경 파일이 세 번 늘었다.** 표면을 손으로 세다가 매번 놓쳤기 때문이다 — 그래서 phase-1 의
소스 가드가 "등록부에 없는 사람-결정 인자를 내는 코드"를 잡는 형태여야 한다(목록 대조가 아니라).
