...
### D1. 통제점을 **세 개**로 쪼갠다 (하나의 "push 직전"이 아님)
| 통제점 | 트리거 | 승인 문구 |
|---|---|---|
| **통합(integration)** | protected branch에 변경을 넣기 직전 | 기본은 PR 생성 승인 |
| **bypass** | required checks를 우회하는 direct push가 필요할 때 | "branch protection bypass를 사용한 direct push" — 이 문구가 없으면 승인 아님 |
| **릴리즈(release)** | tag push · `npm publish` · GitHub release | CI green **이후** 별도 승인. 통합 승인과 절대 묶지 않음 |

세 통제점을 한 줄로 합치면 REQ-2026-007식 확대 해석이 다시 발생한다. **분리 자체가 이 티켓의 산출물**이다.

### D2. "승인 범위 해석 규칙"을 명문화한다
AGENTS.template.md에 규칙 한 줄을 추가한다: **승인은 승인받은 문장 그대로만 유효하다.** "main merge/push 승인"은 required checks bypass 승인이 아니고, 통합 승인은 릴리즈 승인이 아니다. 모호하면 확대 해석하지 말고 다시 물어본다.
...
- 주 레시피: 릴리즈 브랜치 → `npm version --no-git-tag-version` → 커밋 → push(**feature branch**) → PR → required checks green → PR merge.
- tag / publish / release는 **레시피에서 분리된 별도 섹션**으로 옮기고, 각각 "별도 승인 필요"를 명시.
- direct push는 "⚠️ 예외 — bypass 명시 승인 필요" 박스에만 남기고, `remote: Bypassed rule violations`가 우회의 신호라는 것을 적는다.
