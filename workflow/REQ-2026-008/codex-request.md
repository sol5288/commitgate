...
## 정책 개정 (2026-07-09, 사용자 결정 — phase 2)
초판은 "**기본 통합 경로는 PR 경유**"라고 못 박았다. 이 프로젝트는 **1인 개발**이므로 그 강제를 없앤다.
- **PR은 선택 경로(A)**, **direct push는 `B1` 승인 시 허용되는 정상 경로(B)**다. B는 더 이상 "예외"가 아니다.
- 완화 대상은 **경로 선택**이지 **투명성**이 아니다. 그대로 유지되는 것:
  - direct push는 required status checks를 **우회**한다 → push **전에** 보고하고 멈춘다.
  - 승인 문장은 `branch protection bypass를 사용한 direct push 승인` 그대로여야 한다.
  - 경로 B에서 **CI는 사후 검증**이다. 그 사실을 숨기지 않는다. CI green 확인 전에는 `R1`로 넘어가지 않는다.
  - tag / publish / release는 여전히 각각 별도 통제점이다.

## 변경 요약(문서 전용, 코드 무변경)
- 통제점을 **6개로 분리**하고 각각에 **정확한 승인 문장과 시점**을 준다(design R1 P2 반영):
  - `I1` = `feature branch push + PR 생성 승인`  (경로 A, 선택)
  - `I2` = `required checks green 확인 후 PR merge 승인` (경로 A) — **I1과 별개**. I1 승인만으로 머지하지 않는다. green 전 선승인 금지.
  - `B1` = `branch protection bypass를 사용한 direct push 승인` (경로 B) — A를 대체하는 **정상 경로**. 우회 사전 보고 + CI 사후성 명시 의무.
  - `R1`/`R2`/`R3` = `tag 생성·push 승인` / `npm publish 승인` / `GitHub release 생성 승인` — 서로도 독립. 경로와 무관하게 **CI green 이후**.
...
- **정책 개정이 정확히 반영됐는가**: 네 문서 어디에도 PR을 "기본 필수"로 서술하지 않는가(경로 선택으로 읽히는가). 그러면서도 direct push의 **required status checks 우회 사실**과 **CI 사후성**이 약화되지 않고 그대로 명시돼 있는가. 경로 B에서 `R1`(tag) 이전에 CI green 확인이 요구되는가. 완화가 투명성까지 완화해 버리지 않았는가.
