...
- **phase-4(커밋 `1e60e4d`, 통합됨)** — `req:new` marker + `req:next` DONE 직전 HEAD blob 검증 → BLOCKED.
- **phase-5(현재 리뷰 대상)** — 문서(한/영)·CHANGELOG·0.9.7→0.9.8 + phase-4 관찰 대응(`req:next` main() 배선 통합 테스트). 각 phase는 앞 phase의 산출물을 **전제**로 하며 그 코드는 이 diff에 없다.
