...
> 💳 **되돌아가는 화살표가 무한 반복은 아닙니다.** 판정이 나온 리뷰(승인·수정 요청처럼 결과가 정상적으로 나온 리뷰)가 **5회**를 채우기 전까지는 자동으로 돕니다. 채운 뒤에는 설정에 따라 **사람이 예외를 승인**하거나 그대로 진행합니다. **어느 쪽이든 실제 호출이 8회에 도달하면 다음 리뷰는 실행되지 않습니다.** 리뷰는 유료 호출이라 상한이 있습니다 — 값과 정책은 [설정](https://github.com/sol5288/commitgate/blob/main/docs/configuration.md)에서 조정합니다.
...
| 명령 | 용도 |
|---|---|
| `npx commitgate setup` | 리뷰 모델·추론강도·멈춤 지점(`stopGate`)·리뷰 예산 초과 정책(`reviewBudget.onSoftLimit`) 선택 + codex 로그인 (대화형·필수) |
