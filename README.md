...
- req:commit --run 직전
- [I1] feature branch push + PR 생성 직전
- [I2] required checks green 확인 후 PR merge 직전
- [B1] protected branch에 direct push가 필요할 때 — "branch protection bypass를 사용한 direct push 승인"을 따로 받는다
- [R1/R2/R3] tag 생성·push / npm publish / GitHub release — 각각 별도 승인
...
- phase-2: 테스트와 회귀 확인
통제점: req:commit --run 직전 / [I1] PR 생성 직전 / [I2] required checks green 후 PR merge 직전
```

이후에는 에이전트가 설계, 구현, 테스트, Codex 리뷰를 진행합니다. 사용자는 통제점에서만 확인하면 됩니다.

기본 통합 경로는 **PR 경유**입니다. protected branch로 직접 push하려면 별도로 "branch protection bypass를 사용한 direct push 승인"이 필요합니다 — bypass 권한이 있다는 사실은 승인이 아닙니다. tag, npm publish, GitHub release는 merge와 묶이지 않는 별도 통제점입니다. 자세한 계약은 [AGENTS.template.md](AGENTS.template.md)와 [docs/RELEASING.md](docs/RELEASING.md)를 참고하세요.
