...
- 전량 검증의 **자리**(통합 직전)는 `stopGate` 값과 무관하다. `auto` 에서 사람이 다시 승인하지 않더라도 그 자리는 그대로다 — 사라지는 것은 사람의 확인이지 검증이 아니다.
...
- **정지 지점은 `req.config.json`의 `stopGate` 하나가 정한다**(`userConfirmGate`):
  ...
  **`auto`=`merge`와 커밋 단계가 같고, 통합 지점만 다르다**(바로 아래).
  LOW phase는 파생된 `phaseCommit.autoApprove`가 `low-only`일 때 사람 정지 없이 자동 커밋된다
  (`req:next`가 `req:commit --run`을 RUN으로 지시).
- **통합(main 병합) 승인 — `stopGate` 값에 따라 다르다.**
  - `phase`·`req`·`merge` — 통합 승인이 **항상** 필요하다. 커밋에서 멈추지 않게 설정해도 사람 확인은
    feature→main **통합 통제점**(I1/I2/B1)으로 모인다 — `req:next` 종단이 `DONE`이 아니라
    `AWAIT_HUMAN`(통합)으로 멈춘다.
  - `auto` — **유효한 사전 위임이 없으면 `merge`와 똑같이 멈춘다.** 유효한 위임이 있으면 사람이 다시
    승인하지 않고 통합한다(그것이 위임의 목적이다). 위임은 사람이 `req:delegate`로 **미리** 발급하고
    **정확히 한 번** 소비된다 — 에이전트가 스스로 발급하지 않는다.
  - 🔴 **`auto`에서도 멈추는 것**: HIGH 위험을 위임하지 않았을 때 · 리뷰 예산 `hardCap` 도달 ·
    리뷰 `BLOCKED` · 위임 범위 밖 변경이 섞였을 때 · 위임이 만료·철회됐거나 이미 소비됐을 때.
    그리고 티켓의 정지 정책은 **만들어질 때 확정**되므로(정책 스냅샷), 나중에 설정을 `merge`로 바꿔도
    `auto`로 만든 티켓은 여전히 위임을 요구한다.
  - 🔴 **통합 대상을 브랜치 이름에서 확정할 수 없으면**(`delivery/<slug>` 도 `<branchPrefix><연도>-<번호>-…`
    도 아닌 이름) 도구는 **자동 통합하지 않는다.** 이 경우에**만** 대화형 최종 확인으로 사람이 직접
    통합할 수 있다(기본 No). 🔴 **다른 거부 사유는 사람 확인으로도 열리지 않는다** — 위임 부재·만료·
    철회·이미 소비 · trunk 이동 · source 불일치 · 위임 범위 밖 변경 · HIGH 미위임 · `hardCap` ·
    리뷰 `BLOCKED`. 이 경로는 위임이 없으므로 **push 도 하지 않는다**(로컬 병합까지).
  - 🔴 **`auto` 의 권한 판단은 설정값이나 브랜치 이름만으로 내려지지 않는다.** 도구는 통합 대상
    SHA 의 트리에서 **그 티켓의 정책 스냅샷**을 읽고, 브랜치 이름이 표준 형식이 아니면 **범위의 커밋
    귀속**에서 대상을 찾는다. 대상 티켓·묶음을 확정할 수 없으면 **자동 통합하지 않는다**(비대화형은
    거부, 대화형은 사람이 최종 확인에서 판단).
  - 🔴 이 축(`stopGate`)은 **어디서 사람이 확인하는지**만 정한다. 리뷰가 길어질 때 비용을 자동으로
    허용할지는 **다른 축**(`reviewBudget.onSoftLimit`)이고, 둘 중 어느 것도 `hardCap`을 해제하지 않는다.
...
묻기만 하고 기다리는 것은 진행을 멈추는 것과 **같은 효과**다. "안전 쪽으로 기울었다"는 이유가 되지 않는다 —
사용자가 `stopGate`로 정한 정지 지점이 세션마다 늘어나는 것이기 때문이다. 같은 설정에서 같은 워크플로가
세션마다 다르게 끊기면 그 설정은 의미를 잃는다.

**통제점이 아닌 판단은 권장안으로 진행한다.** `stopGate`가 **`phase`를 제외한 모든 값**
(`req`·`merge`·`auto`)일 때, 아래 예외에 해당하지 않는 판단(설계 선택지·구현 방식 등)은
**권장안을 택하고 그 근거를 `01-design.md`에 남긴 뒤** 계속한다.
`stopGate: "phase"`에서는 이 자율 규칙을 적용하지 않는다 — 그 값을 고른 사용자는 자주 멈추기를 원한 것이다.

🔴 `auto`는 이 규칙에서 **빠지는 값이 아니라 가장 강하게 해당하는 값**이다. `merge`보다 더 자율적인
지점을 고른 사용자에게 더 자주 묻는 것은 그 설정을 무의미하게 만든다.

🔴 **예외는 `kind`가 아니라 _행위_로 판정한다.** `req:next`는 `AGENT`를 내면서 확인이 필요한 조치를
**진단 줄로** 함께 낼 수 있다(`req:rebind`가 그렇다). `kind`만 보고 "묻지 않는다"로 읽으면 사람 판단을
에이전트가 대신 쓰게 된다.
...
| 6 | 필수 전제 미충족(git·Codex CLI·Node·패키지매니저 — fail-closed) |
| 7 | `req:next`가 낸 `AWAIT_HUMAN`·`BLOCKED` |
| 8 | 사람 전용 명령(`commitgate setup`) |
| 9 | **확인 문장(`--confirm`)을 요구하는 명령** — `req:rebind` · `delivery seal`/`approve`/`reopen` |
...
| 명령 | 이유 | 에이전트가 할 일 |
|---|---|---|
| `npx commitgate setup` | **대화형 전용**이다. 리뷰 모델·추론강도·멈춤 지점(`stopGate`)·리뷰 예산 초과 정책(`reviewBudget.onSoftLimit`)을 묻고 `codex login`을 실행한다. 비대화형에서 실행하면 아무 질문도 하지 않고 즉시 종료한다 | 사용자에게 *"터미널에서 `npx commitgate setup`을 실행해 주세요"* 라고 **요청**한다 |

이 규칙의 목적은 편의가 아니다. setup은 **두 정책 축**을 정한다 — **어디서 사람이 확인하는지**
(`stopGate`)와 **리뷰 예산을 넘겼을 때 사람 승인 없이 계속할지**(`reviewBudget.onSoftLimit`). 에이전트가
스스로 실행할 수 있으면 **자기가 통과할 게이트를 자기가 고르는** 경로가 열린다. 명령 자체도 비대화형에서
fail-closed로 거부하지만, **계약이 먼저 막는 것이 옳다.**
...
- 설계 **범위 변경** 또는 비목표 추가 필요 — 아래 경계를 먼저 판정한다
- Codex 리뷰 BLOCKED(exit 2) 또는 제한된 재시도 후 판단 불명확
- git·Codex CLI·Node·패키지매니저 등 필수 전제 미충족(fail-closed)
- 확인 문장(`--confirm`)을 요구하는 명령 실행 직전(`req:rebind` · `delivery seal`/`approve`/`reopen`)

🔴 **설계 "정정"과 "범위 변경"은 다르다.** 이 경계가 없으면 §4-1의 자율 진행과 위 규칙이 정면 충돌한다.

| 무엇 | 판정 | 처리 |
|---|---|---|
| **정정** — 같은 목표·같은 완료 기준을 유지한 채, 리뷰 지적이나 실측 반증에 따라 **방법**을 고침 | 자율 | 고치고 `req:next`가 지시하는 **설계 재승인**을 태운다(재승인 자체가 리뷰어 게이트다) |
| **범위 변경** — `00-requirement.md`의 **무엇 / 완료 기준**이 바뀌거나 비목표가 목표가 됨 | **사람** | 멈추고 보고한다 |

판정 기준은 하나다: **`00-requirement.md`를 고쳐야 하는가.** 고쳐야 하면 범위 변경이다.
