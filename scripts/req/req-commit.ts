...
// 🔴 REQ-2026-175: `terminalReentryProblem` 은 `lib/terminal-reentry` 로 이관됐다 —
//    `req:next` 가 **같은 판정·같은 입력**으로 안내를 내야 하기 때문이다(안내와 거부가 갈라지지 않게).
//    re-export 로 기존 import 경로·테스트를 그대로 둔다.
import { terminalReentryProblem, computeTerminalReentry } from './lib/terminal-reentry'
export { terminalReentryProblem }
...
export function main(argv: string[] = process.argv.slice(2)): void {
  ...
  {
    /**
     * 🔴 판정과 **입력 획득**을 `lib/terminal-reentry` 가 통째로 갖는다(REQ-2026-175).
     *    `req:next` 가 **같은 함수**로 안내를 내므로, 안내와 거부가 갈라질 수 없다 —
     *    예전에는 `req:next` 가 `RUN` 으로 지시한 명령을 이 자리가 거부했다.
     */
    const reentry = computeTerminalReentry({ root: cfg.root, ticketRel, reqId: String(state.id ?? ''), git })
