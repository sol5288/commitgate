...
> 대부분 자체 감사에서 나온 **내부 정리**로, 소비자가 관측할 수 있는 변화가 없습니다 —
> CLI 표면·리뷰 프롬프트·`state.json` 스키마·원장/증거 아카이브 형식이 모두 그대로입니다.
> **예외는 REQ-2026-107 하나로, `req:doctor`의 D18 출력이 바뀝니다**(오탐이 사라지는 방향).
