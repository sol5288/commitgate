...
- **docs+feat: `auto` 정책의 범위를 확정하고 기존 프로젝트의 업그레이드 경로를 열었습니다** —
  ...
  - 🔴 **`stopGate: "auto"`(최종 merge까지 무정지)는 두지 않습니다.** 통합 승인은 세 값 공통 불변식이고,
    도구가 확인 기록을 대신 만드는 방식은 시각 날조 표면을 되살립니다. 정직한 유일한 형태(작업 시작
    시점의 사전 위임 기록)는 안전 속성 변경이라 별도 논의가 필요합니다 — 그 근거를 문서에 남겼습니다.
