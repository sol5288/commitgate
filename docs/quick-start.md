...
**네 가지**를 묻고, 각 질문은 **↑/↓로 고르고 Enter로 확정**합니다(Ctrl+C 취소). 값은 `req.config.json`에 기록됩니다.
...
| 멈춤 지점 (`stopGate`) | `req` | `phase` · `req` · `merge` · `auto` |
| 리뷰 예산을 넘겼을 때 (`reviewBudget.onSoftLimit`) | `ask` | `ask` · `auto` |

- **모델**은 목록에 없는 값을 쓸 수 있어 "직접 입력…" 항목이 있습니다. 나머지 세 질문은 스키마 enum이라 목록에서만 고릅니다.
- **비움**은 값을 지우는 것이 아니라 *codex 전역 설정을 따르겠다*는 뜻입니다.

#### 🔴 마지막 두 질문은 **서로 다른 것**을 정합니다

헷갈리기 쉬운 지점입니다. 둘 다 "멈춘다"는 말을 쓰지만 **다루는 것이 다릅니다.**

| 질문 | 무엇을 정하나 | 성격 |
|---|---|---|
| 멈춤 지점 (`stopGate`) | **사람이 승인할 자리** — 커밋·통합 | 🔒 **안전** |
| 리뷰 예산 (`reviewBudget.onSoftLimit`) | 재리뷰가 **예산을 넘겼을 때** 계속할지 | 💳 **비용** |

- **멈춤 지점**이 사람 확인 위치를 정합니다 — `phase`=매 phase 커밋 전 · `req`=REQ를 완성시키는 커밋 · `merge`=묶음(delivery set) 종료(묶음이 없으면 `req`처럼 REQ 통합 직전) · `auto`=`merge`와 같되 **사람이 미리 발급한 위임이 있을 때만** 통합에서도 멈추지 않습니다(위임이 없으면 그대로 멈춥니다). 위험도 `HIGH` 취급을 포함한 정본은 [워크플로 — HIGH 위험 티켓의 사람 확인](./workflow.md#high-위험-티켓의-사람-확인)입니다.
- **리뷰 예산**은 같은 곳을 몇 번까지 자동으로 다시 리뷰할지를 정합니다. 판정이 나온 리뷰가 `autoBudget`(기본 5)회를 채우기 전까지는 자동입니다. 채운 뒤가 이 질문이 정하는 구간입니다:
  - `ask`(기본) — 그다음부터는 **회차마다 사람이 예외를 승인**해야 진행합니다.
  - `auto` — 사람 승인 없이 계속 진행하고, 원장에 그 근거를 남깁니다.

  🔴 **`auto`를 골라도 위 "멈춤 지점"이 정한 사람 확인(커밋·통합 승인)은 그대로입니다.** 다만 **예산을 넘긴 뒤 요구되던 "사람 예외 승인"은 없어집니다** — 그만큼 사람이 개입하지 않고 리뷰 비용을 더 쓰게 됩니다.

#### 🔴 `auto` 는 **두 곳**에 있습니다 — 뜻이 다릅니다

| 어디의 `auto` | 뜻 |
|---|---|
| 멈춤 지점 `stopGate: "auto"` | **사람이 미리 발급한 위임이 있을 때만** 통합까지 자동 진행 |
| 리뷰 예산 `reviewBudget.onSoftLimit: "auto"` | 예산 초과 뒤 **사람 예외 승인 없이** 재리뷰 진행 |

🔴 **둘 다 `hardCap`(기본 8)을 해제하지 않습니다.** 이름이 같을 뿐 다루는 것이 다릅니다.

  🔴 **어느 값을 골라도 `hardCap`(기본 8)을 채우면 차단됩니다.** 두 한도는 **세는 기준이 다릅니다** — `autoBudget`은 **판정이 나온 리뷰**만 세고, `hardCap`은 **실제로 나간 호출**을 셉니다. 응답이 무효(invalid)면 판정 횟수에는 들어가지 않지만 호출 횟수에는 들어갑니다(돈은 이미 나갔으니까요). 자세히는 [설정 — `reviewBudget`](./configuration.md#리뷰-예산--reviewbudget).
...
> 한 series의 재리뷰 횟수에는 상한이 있습니다. 기본값에서는 **판정이 나온 리뷰 5회**까지 자동이고, 그 뒤는 `reviewBudget.onSoftLimit`이 정합니다(`ask`=회차마다 사람 예외 승인 · `auto`=승인 없이 진행). **어느 값이든 실제 호출 8회를 채우면 차단**됩니다 — [설정 — `reviewBudget`](./configuration.md#리뷰-예산--reviewbudget)을 보세요.
