...
| `reviewBudget` | `{ "autoBudget": 5, "hardCap": 8, "onSoftLimit": "ask" }` | Re-review attempt budget for an open `(review_kind, phase_id)` review series. With the defaults, **reviews that produced a verdict** run automatically until they reach `autoBudget`. After that `onSoftLimit` governs: `"ask"` (default) requires a human exception record bound to that series and round; `"auto"` runs without human approval and records the policy grounds in the ledger. Once `hardCap` **calls actually made** are spent the next attempt is blocked **under both values**. 🔴 The two limits count different things — an invalid response is excluded from the `autoBudget` tally but included in the `hardCap` tally, so a fixed round number cannot be quoted. `hardCap ≤ 8`, `autoBudget ≤ hardCap`. See [Review budget](#review-budget--reviewbudget) |
| `stopGate` | `"req"` | **Decides where a human stops at commits and integration** (preferred axis). 🔴 The review budget (`reviewBudget.onSoftLimit`) is a **separate axis**: regardless of this value, exceeding it during re-reviews can stop you on its own. `phase` = confirm before every phase commit; `req` = auto-commit phases inside a REQ and gather the confirmation at **the commit that completes the REQ**; `merge` = group several REQs into a delivery set and defer until **the whole set** is done (🔴 **with no delivery set it behaves like `req`**, stopping just before this REQ's integration — choosing this value does not remove the stop); `auto` = same as `merge`, except that **a valid pre-delegation** also carries it through integration (with no delegation it stops exactly as `merge` does — see [`stopGate: "auto"`](#stopgate-auto--automatic-integration-of-verified-changes-within-a-delegated-scope)). The per-value confirmation points, including how `HIGH` risk is treated, are defined in [Workflow — Human confirmation for HIGH-risk tickets](workflow.en.md#human-confirmation-for-high-risk-tickets). Integration (main merge) approval is always required under `phase`, `req`, and `merge`; under `auto` a **valid advance delegation stands in for that approval** (without one, it is still required) |
...
- `autoBudget` (default 5): **reviews that produced a verdict** repeat with no human involvement until they reach this count.
- `hardCap` (default 8): the **absolute call ceiling**. Once this many **calls have actually been made**, the next attempt never runs, by any route.
- 🔴 **The two values count different things.** An invalid response is excluded from the `autoBudget` tally but included in the `hardCap` tally — the call already went out and cost money. So a fixed "from round N you need an exception" cannot be quoted (one invalid response pushes it back).
- `onSoftLimit` (default `ask`): what happens once `autoBudget` is exceeded.
  - `ask`: each attempt past `autoBudget` needs a human approval via `req:review-exception` (current behaviour).
