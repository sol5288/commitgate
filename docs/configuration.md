...
#### `stopGate: "auto"`(최종 merge까지 무정지)는 없습니다

의도적으로 두지 않았습니다. 통합 승인은 세 `stopGate` 값 **공통 불변식**이라, 값 하나가 그것을 없애면
계약이 설정에 종속됩니다. 도구가 확인 기록을 대신 만드는 방식은 시각 날조 표면을 되살리므로 택하지
않습니다(`user_commit_confirmed`는 `req:confirm`만 씁니다).

정직하게 구현할 수 있는 유일한 형태는 **작업 시작 시점의 사전 위임 기록**(사람이 실제 시계로 위임 문장을
남기고 도구가 그 범위 안에서만 자동 병합)입니다. 이는 **안전 속성 변경**이라 별도 논의가 필요합니다 —
필요해지면 그때 다루면 됩니다.
