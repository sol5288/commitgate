...
- `autoBudget`(기본 5): **판정이 나온 리뷰**가 이 횟수를 채우기 전까지는 사람 개입 없이 반복됩니다.
- `hardCap`(기본 8): **절대 호출 상한**. **실제로 나간 호출**이 이 횟수를 채우면 다음 시도는 어떤 경로로도 실행되지 않습니다.
- 🔴 **두 값은 세는 기준이 다릅니다.** 응답이 무효(invalid)면 `autoBudget` 쪽 계수에는 들어가지 않지만 `hardCap` 쪽에는 들어갑니다 — 호출은 이미 나갔고 비용도 발생했기 때문입니다. 그래서 "N회차부터 예외가 필요하다"고 고정해 말할 수 없습니다(무효 응답이 한 번 있으면 그만큼 뒤로 밀립니다).
- `onSoftLimit`(기본 `ask`): `autoBudget`을 넘긴 뒤 무엇을 할지.
  - `ask`: 넘긴 회차마다 `req:review-exception` 사람 승인이 필요합니다(현행).
