...
```mermaid
flowchart LR
    Install["npm install -D commitgate<br/>npx commitgate init<br/>(2단계 설치)"] --> New["req:new --run<br/>(티켓·브랜치)"]
...
## 2. 화면 A — 설치 `npm install -D commitgate` → `npx commitgate init`

**Stage B 런타임 패키지 모델이라 설치는 2단계다.** 먼저 런타임을 devDependency로 설치하고(`npm install -D commitgate`), 그다음 관리 자산을 배치한다(`npx commitgate init`). 실행 코드(`scripts/req/**`)는 대상 repo에 **복사되지 않고** `node_modules/commitgate`에서 돈다 — 주입된 `req:*`가 그리로 dispatch한다([bin/init.ts](../../bin/init.ts) `planInstall`, [bin/dispatch.mjs](../../bin/dispatch.mjs)).

- **진입 조건**: 대상 폴더가 git repo이고 `package.json` 존재. 아니면 fail-closed throw([bin/init.ts](../../bin/init.ts) `assertGitWorkTree`).
- **Stage B 전제 — 순서가 계약이다**([bin/init.ts](../../bin/init.ts) `runInit`): 다음 두 검사를 **D19 → D14 순서로** 돌린다. 둘 다 preflight라 throw 시 **어떤 파일도 쓰이지 않는다**.
  1. **D19 — Stage A 서명 감지**: `req:*` 값이 정확히 Stage A 주입값이거나 `scripts/req/`가 존재하면 감지 근거와 함께 throw → **`commitgate migrate` 안내로 중단**(화면 H). plain init은 기존 `req:*`를 덮지 않으므로, 막지 않으면 **런타임은 vendored인 채인데 사용자는 Stage B라 믿는 조용한 혼합 설치**가 된다([bin/init.ts](../../bin/init.ts) `detectStageA`).
  2. **D14 — 선행 설치 확인**: `devDependencies.commitgate` **키 존재만** 본다. 없으면 throw → **`npm install -D commitgate` 선행 설치 안내로 중단**. **값의 형태는 검증하지 않는다** — `npm i -D <tgz>`는 `file:…tgz`를, `link:`/`workspace:`/git URL도 정당한 설치 형태다([bin/init.ts](../../bin/init.ts) `commitgateDeclared`).
  - 순서를 뒤집으면 안 된다: Stage A 설치본에는 `devDependencies.commitgate`가 **없으므로** Stage A 사용자가 항상 D14에서 먼저 죽어 migrate 안내에 **영원히 도달하지 못한다**(design r20 P1 — 회귀 테스트가 이 순서를 고정).
- **입력(플래그)**: `--dir <path>`(기본 cwd), `--force`, `--dry-run`, `--strict`, `--no-agent-entrypoints`, `-h/--help`. 알 수 없는 인자 → throw. `init` verb는 생략 가능하다 — argv가 없거나 첫 인자가 `-`로 시작하면 dispatch가 argv 전체를 init에 넘긴다([bin/dispatch.mjs](../../bin/dispatch.mjs) `resolveDispatch`).
- **설치하는 것**: 관리 자산(workflow 스키마 2종 · reviewer persona · `req.config.json` · `AGENTS.md`/`CLAUDE.md`·에이전트 진입점)과 `package.json`의 `req:*` = `commitgate <verb>` — **없는 키만** 주입한다(`if (!(k in scripts))`, 기존 값·사용자 정의 값 보존).
- **설치하지 않는 것**: **`scripts/req/**` 실행 코드**(무복사)와 **`tsx`·`ajv`·`cross-spawn` devDependency**(무주입). 전부 `commitgate` 패키지 안에 있고 `npm i -D commitgate` 시 전이 설치된다 — 대상의 `devDependencies`는 건드리지 않는다([bin/init.ts](../../bin/init.ts) `runInit`, `printHelp`).
...
- **오류/경고**: Stage A 서명 감지(throw → migrate), `devDependencies.commitgate` 미선언(throw → 선행 설치), cross-spawn 하한 미달(WARN, `--strict`면 throw), 계약 포인터 gitignore(WARN/throw), 워킹트리에 안전 커밋 불가(`--strict`면 throw). **파일을 하나도 쓰기 전에** 프리플라이트에서 중단.
- **접근 제어**: 없음(로컬).
...
- **D19 — 설치 모드 진단**: `package.json`의 `req:*` **값의 형태만** 보고 5종으로 분류해 한 줄로 보고한다([scripts/req/req-doctor.ts](../../scripts/req/req-doctor.ts) `classifyInstallMode`).
  ...
  | 상태 | 표시 | level |
  |---|---|---|
  | `stage-a` | `설치 모드: Stage A(vendored — scripts/req/** 를 직접 실행)` | OK |
  | `stage-b` | `설치 모드: Stage B(런타임 패키지 — commitgate <verb> dispatch)` | OK |
  | `mixed` | Stage A·B 형태가 섞임 → **`commitgate migrate` 로 전환하세요** | **WARN** |
  | `none` | `설치 모드: req:* 스크립트 없음` | OK |
  | `custom` | `설치 모드: 사용자 정의 req:* 값(kit 형태 아님)` | OK |

  `scripts`를 읽지 못했거나 없으면 `점검 불요` OK.
- **D19는 절대 FAIL이 아니다**(level 상한 WARN — `mixed`만 WARN, 나머지는 OK): Stage A는 결함이 아니라 지원되는 설치 형태이고, CommitGate 자신의 `package.json`이 Stage A 형태다. `req:commit`이 이 doctor를 exit≠0에 throw하는 **하드 게이트**로 spawn하므로, FAIL로 올리면 이 저장소 자신과 정당한 Stage A 소비자 전원의 커밋이 **영구 차단**된다([scripts/req/req-doctor.ts](../../scripts/req/req-doctor.ts) D19 블록).
- **D19의 판정 범위**: `req:*` 값의 형태뿐이다. lockfile·`node_modules`·설치 버전·자산↔런타임 skew를 **보지 않는다**. [bin/init.ts](../../bin/init.ts)를 import하지 않으므로(레이어 역전 방지) 바이트 일치가 아니라 shape 판정이며, 이 비대칭은 의도적이다 — **쓰기**를 하는 init preflight·migrate는 바이트 정확 일치를, **읽기 전용 advisory**인 이 진단은 shape를 쓴다.
...
- **진입 조건**: git repo. **읽기 전용 안내 전용 — 아무 것도 지우지 않는다**([bin/uninstall.ts](../../bin/uninstall.ts) `renderPlan`).
- **표시**: 6개 섹션 + npx 캐시 안내 — §1 CommitGate 소유 파일(removable `-`/review `~`/unknown `?`), §2 자동 제거 대상 아님(ambiguous keep), §3 감사 증거(protect, 삭제 금지), §4 되돌리는 방법(commit revert / 미커밋 checkout), §5 **런타임 패키지 제거(Stage B)**, §6 잔여물 경고.
- **런타임 제거는 사용자가 별도로 한다**: Stage B에서 실행 코드는 이 저장소가 아니라 `node_modules/commitgate`에 있으므로 제거는 package manager의 책임이다. `devDependencies.commitgate`가 선언돼 있으면 **`npm uninstall -D commitgate`**(pnpm: `pnpm remove -D commitgate` · yarn: `yarn remove commitgate`)를 사용자가 직접 실행하도록 안내한다. 이 명령은 **문자열로 출력만 하고 npm을 spawn하지 않는다**([bin/uninstall.ts](../../bin/uninstall.ts) `renderRuntimeRemovalSection`). `package.json`의 `req:*`도 고치지 않는다(수동 정리 후보로 표시만).
- **입력**: `--dir <path>`만. `--run`/`--force`/`--apply` **플래그 부재가 계약**이다(원장이 없어 blind 삭제 시 사용자 데이터 파괴 위험). 알 수 없는 인자 → throw([bin/uninstall.ts](../../bin/uninstall.ts) `parseArgs`).

## 9. 화면 H — 전환 `npx commitgate migrate`

Stage A(vendored scaffold) 설치본을 Stage B(런타임 패키지)로 옮기는 경로다. 화면 A의 D19가 여기로 보낸다([bin/migrate.ts](../../bin/migrate.ts)).

- **진입 조건**: 대상이 git repo이고 `package.json` 존재. 아니면 throw([bin/migrate.ts](../../bin/migrate.ts) `planMigrate` — `assertGitWorkTree`).
- **입력**: `--dir <path>`(기본 cwd), `--apply`, `--dry-run`(기본값의 명시 표현), `-h/--help`. 알 수 없는 인자 → throw([bin/migrate.ts](../../bin/migrate.ts) `parseArgs`). 대상 root를 `--dir`로만 해소하는 것이 계약이다 — config 탐색 fallback을 타면 CommitGate 패키지 **자신의** `package.json`을 재작성한다.
- **하는 일(전부)**: `package.json`의 `req:*` 중 **현재 값이 정확히 Stage A 주입값인 키만** `commitgate <verb>`로 바꾼다. 그 외에는 아무것도 하지 않는다([bin/migrate.ts](../../bin/migrate.ts) `decideScripts` — 키별 판정 `convert`/`stage-b`/`custom`/`absent`).
- **기본 dry-run(쓰기 0건)**: `--apply` 없이는 계획만 출력한다. `--apply`의 쓰기 범위는 **`package.json` 한 파일**이며 **커밋하지 않는다** — 사용자가 `git diff package.json`으로 검토한 뒤 직접 stage/commit한다([bin/migrate.ts](../../bin/migrate.ts) `runMigrate`).
- **`--apply` 전제**: `devDependencies.commitgate` 키 존재 확인. 없으면 throw → `npm install -D commitgate` 안내. 화면 A의 D14와 **같은 축소 규칙**(키 존재만, 값 형태 미검증)을 쓴다.
- **표시**: 대상 root → 전환 대상 키별 before/after(`package.json#scripts.req:new: "tsx scripts/req/req-new.ts" → "commitgate req:new"`) → 이미 Stage B인 키 → ⚠️ 사용자 정의 값 → ℹ️ 잔존 `scripts/req/` 안내 → 다음 명령. 전환 대상이 없으면 `전환 대상 없음`([bin/migrate.ts](../../bin/migrate.ts) `renderPlan`).
- **사용자 정의 값은 덮어쓰지 않는다**: 한 글자라도 Stage A 주입값과 다르면 사용자 값으로 보아 **보존하고 수동 조치를 안내**한다(“그 스크립트가 `commitgate <verb>` 를 호출하도록 직접 고치십시오”).
- **비파괴 — 아무것도 삭제하지 않는다**: `scripts/req/**`·스키마·persona·`req.config.json`·진입점·`workflow/REQ-*` 증거를 **자동 삭제하지 않는다**. 잔존 vendored 파일은 Stage B에서 실행되지 않을 뿐이며, 정리는 읽기 전용 `commitgate uninstall` 계획(화면 G)이나 `git revert` 안내로만 유도한다.
- **오류**: 대상 디렉터리 부재, `package.json` 부재·파싱 실패, `scripts`/`devDependencies`가 객체 아님 → 전부 쓰기 전 throw(fail-closed).
- **접근 제어**: 없음(로컬).
...
| 설치 | 런타임 무복사(실행 코드는 패키지에), 기존 파일·사용자 정의 `req:*` 보존, Stage A는 fail-closed로 막고 `migrate`로 유도 | 2단계(`npm i -D` → `init`)로 늘어남. 설치 원장·업그레이드 plan 없음. **자산↔런타임 버전 skew를 자동 감지할 수단이 없다**(D19는 `req:*` 형태만 본다) |
