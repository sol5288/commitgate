...
| **D31** | phase 실효 위험(민감 경로) | **(WARN 상한, FAIL 아님) 없음.** staged 경로가 민감 패턴(config `riskPaths` — `null`=내장 기본 목록·`[]`=비활성, 소문자 부분 문자열 매치)에 일치하면 패턴·대표 경로와 함께 알린다 — 티켓 위험도(`state.risk_level`)는 생성 시 입력값이라 phase의 실효 위험과 갈릴 수 있다. **강제하지 않는다**: 확인 강제는 발화율 데이터가 쌓인 뒤 별도 REQ의 결정이다(0.13.0 block→warn 정정·REQ-2026-066 선례). staged 없음·비활성은 점검 불요(OK). 실행 로그에는 id·level만 남는다(경로는 subjects 허용 목록 밖) (REQ-2026-119) |
| **D32** | 정지 정책 드리프트 | **(WARN 상한, FAIL 아님) 없음.** 티켓에 고정된 `state.policy_snapshot.stop_gate`가 `req.config.json`의 `stopGate`와 다르면 알리고 `req:repolicy` 채택 명령을 안내한다. 네 경우를 구분한다: 일치(OK) · 스냅샷 없음=legacy(OK — config를 따름) · 손상(WARN — config로 폴백함을 알림) · 불일치(WARN). **FAIL이 아닌 이유**: 정책 변경은 정당한 행위이고 여기서 막으면 진행 중 티켓이 전부 교착한다. 게이트는 이미 스냅샷을 쓰므로 판정은 일관하며, 필요한 것은 차단이 아니라 가시성이다 (REQ-2026-129) |
| **D33** | 설치본 `req:*` 명령 표면 skew | **(WARN 상한, FAIL 아님) 없음.** 대상 repo `package.json`의 `scripts` 키 집합이 설치된 패키지의 verb 표면(`bin/dispatch.mjs`의 `VERB_MODULES` 중 `req:*`)보다 좁으면 부족한 verb 이름과 `commitgate sync --apply --scripts` 안내를 낸다. **D19와 질문이 다르다**: D19는 설치 *모드*(Stage A/B/mixed)를 5개 표본 키의 **값 형태**로 판정하고 부재 키는 걸러내므로, 새 verb가 없어도 `OK Stage B`다 — 한 체크에 두 질문을 섞으면 한쪽 답이 다른 쪽을 가린다. 판정·입력 획득·안내 문장은 `commitgate check`의 C6와 **같은 정본**(`lib/command-surface`)을 쓴다. dev repo/dogfood(packageRoot === config root)·`scripts` 판독 실패·미계산은 점검 불요(OK). **FAIL이 아닌 이유**: 스크립트 부재로 기존 커밋 경로가 벽돌이 되면 안 된다 (REQ-2026-161) |
| **D34** | 열린 orphan review series | **(WARN 상한, FAIL 아님) 없음.** `state.review_series` 중 `review_kind='phase'` 이고 그 `phase_id` 가 `phases[]` 에 **없으면서** `closed_reason: null` 인 것을 series id·사라진 phase id와 함께 알리고 `req:review-exception --close-orphan` 해소 명령을 낸다. 리뷰 지적을 따라 phase를 개명·재정렬하면 옛 series 레코드가 열린 채 남는데, 예전에는 `integrate` 가 이것을 `reviewInconclusive` 로 세어 `stopGate:"auto"` 자율 통합이 **영구 차단**됐다(REQ-2026-161 실측). 이제 판정에서 제외하므로 **통합은 막지 않고**, 부정확한 lifecycle 기록만 알린다 — 판정·진단이 `lib/review-series` 의 **같은 술어**를 쓴다. 미계산은 점검 불요(OK). **FAIL이 아닌 이유**: phase 개명은 정상 행위이고, 막으면 그 티켓의 모든 커밋이 벽돌이 된다 (REQ-2026-163) |
