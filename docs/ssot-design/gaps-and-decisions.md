...
- **해소**: REQ-2026-103이 resume 배선을 어댑터까지 제거했다. **나중에 켜기만 하면 되는 보존 코드로 취급한다는 이 항목의 옛 결론은 철회한다** — 도달 불가 코드를 보존하면 준비된 기능처럼 읽히지만 실제로는 검증된 적이 없다.
...
### G-06b. 승인 후 편집 시 전면 재리뷰 — **해소됨(REQ-2026-031~036)**
- **사실(당시)**: 설계 문서 해시가 **한 글자만 바뀌어도** design 승인 전체가 무효화되고, 재리뷰는 변경분이 아니라 **설계 전체**를 대상으로 다시 돌았다. 스테이트리스 재리뷰는 직전 라운드 findings 스냅샷만 참고하므로, 리뷰어는 자기가 몇 라운드째인지·같은 주제를 몇 번 돌았는지 알지 못한다.
- **근거(당시)**: 실측 — REQ-2026-014는 **r19에서 findings 0건 승인**이 났으나 이후 요구 추가(R10 등)로 해시가 바뀌어 승인이 증발했고, r20~r30이 백지에서 재탐색했다(그 21건은 전부 P2, P1 0건). 승인 시점 대비 요구 60줄→71줄·설계 194줄→251줄로 자라났다.
- **해소(2026-07~08, 개선 B)**: 델타 리뷰가 구현됐다. `design_baseline`(승인 시 저장하는 문서별 blob OID) 대비 변경분만 심사하고, 미변경 본문은 생략 표시로 대체한다([review-codex.ts](../../scripts/req/review-codex.ts) `hasDesignBaseline`·`computeDesignDelta`·`applyDeltaPersona`). 리뷰어 행동 계약(`DESIGN_DELTA_CONTRACT`)이 "표시된 변경분과 직접 영향만 심사, 승인 영역 재litigate 금지"를 건다. 판단 불가 시 `full_review_requested: "yes"`로 전체 재리뷰를 요청하면 baseline이 비워져 다음 라운드가 full로 돌아간다.
- **🔴 남은 관찰 — escalation은 아직 한 번도 쓰이지 않았다(REQ-2026-113 실측)**: 소비자 3개 저장소 271티켓·리뷰호출 2,089건(2026-07-18~08-02)에서 design 호출 955건 중 **델타 모드 241건(25.2%)**, 그 안에서 `full_review_requested: "yes"`는 **0건**이다.
  - **원인은 미확정이다.** 발화 지침은 이미 `DESIGN_DELTA_CONTRACT`에 있고, design 아카이브 902건에서 "문맥이 부족하다"는 지적도 발견되지 않았다. 다른 후보는 **비용 비대칭**이다 — escalation은 `commit_approved: "no"`를 강제하고 baseline을 삭제해 다음 라운드를 전체 재리뷰로 만든다. 판단할 수 있는 리뷰어는 그냥 판단한다.
  - **지침을 늘리지 않기로 했다**(REQ-2026-113). 원인이 지식인지 비용인지 모르는 상태에서 텍스트를 더하는 것은 근거가 없다. 대신 로그에 `delta_mode`·`full_review_requested`를 남겨 **셀 수 있게** 했다. 판단은 데이터가 쌓인 뒤로 미룬다.
  - ⚠️ 그 수치는 원래 `applyDeltaPersona(base, true)`의 `policy_version` 해시를 **역산**해 얻었다. 이제 로그가 직접 답하므로 그 우회는 반복하지 않아도 된다.
...
### G-11. 사용자 가치·비용·수렴 관측 지표 — **부분 해소(원자료는 생겼고 집계 CLI는 없다)**
- **사실(당시)**: 텔레메트리·메트릭·집계 CLI가 없었다. 개별 archive와 exit code는 있으나 온보딩 시간, 리뷰 라운드 P50/P95, 대기 시간, 실패 코드 비율, fresh-clone 복구율을 자동 산출하지 않았다.
- **영향**: P1 전용 차단 정책이나 stateless 재리뷰가 실제로 수렴을 개선했는지 표본 일화 외에는 판단하기 어렵다. 테스트 개수 증가를 사용자 가치로 오인할 위험이 있다.
- **부분 해소 — 로컬 관측 로그 2종이 생겼다**(둘 다 gitignored·워킹디렉터리 상주라 미병합 브랜치와 함께 사라지지 않는다):

  | 로그 | 무엇을 담는가 | 도입 |
  |---|---|---|
  | `workflow/.review-calls.jsonl` | 리뷰 호출 1건 = 1행. 판정·개수·소요·모델·추론강도·프롬프트 바이트·해시·델타 모드·전체재리뷰 요청 | REQ-2026-025 → 043 → 045 → 089 → **113** |
  | `workflow/.doctor-runs.jsonl` | `req:doctor` 실행 1회 = 1행. 전체 판정·평가된 검사 수·**OK가 아닌 검사의 id와 level** | **REQ-2026-111** |

  이 원자료로 이미 답이 나온 질문들이 있다 — 리뷰 게이트 차단률, 라운드 분포, 티켓 시간 중 리뷰 실행 비중, 델타 모드 비율. **내용(프롬프트·diff·finding 본문)은 여전히 담기지 않는다.**
- **여전히 없는 것**: 집계·리포트 CLI, 온보딩 시간, fresh-clone 복구율, 그리고 **두 로그를 함께 보는 수단**. 지금은 사람이 임시 스크립트로 파싱해야 한다.
