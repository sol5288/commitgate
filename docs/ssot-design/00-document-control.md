...
| **D-체크** | `req:doctor`의 일관성 검사. **전수 목록과 각 검사의 FAIL 조건은 [07 §3](07-business-rules-and-state-machines.md)이 정본**이며 여기서 다시 나열하지 않는다(목록이 복제돼 드리프트한 것이 REQ-2026-099의 원인이다). id의 권위는 `req-doctor.ts`의 `D_CHECK_IDS` 등록부다. | [scripts/req/req-doctor.ts](../../scripts/req/req-doctor.ts) `D_CHECK_IDS`·[07 §3](07-business-rules-and-state-machines.md) |
