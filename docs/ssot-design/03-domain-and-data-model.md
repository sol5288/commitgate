...
|---|---|---|---|
| **티켓 상태** `state.json` | `workflow/REQ-*/state.json` | `req:new`(생성), `review-codex`/`req:commit`(변경), 사람(HIGH 확인·수동 최종 보존 사례) | 생성 시 스캐폴드 커밋에 포함되어 **tracked**. 이후 변경은 scratch로 취급되어 정상 source/evidence 커밋에 포함되지 않음(§8). |
...
> **중요 — state JSON Schema는 없다.** `loadState`는 JSON 파싱 뒤 `id`·`phase` 존재만 확인한다. 나머지 필드는 `req:next`의 무결성 검사, doctor D-체크, review/commit 사용 지점에서 부분 검증한다. 따라서 아래 표는 관찰된 전체 필드 계약이지 `workflow/*.schema.json`으로 일괄 강제되는 스키마가 아니다.
...
| `phase` | string | 레거시/거친 코스 마커. 생성 시 `"INTAKE"`; 현재 CLI는 이후 값을 자동 갱신하지 않음 | `"INTAKE"` |
...
> **`phase`와 실제 진행도를 혼동하지 말 것.** main에 추적된 현재 티켓 state들은 완료 이력이 있어도 `phase="INTAKE"`인 사례가 일반적이다. 설계 승인·phase 승인·커밋 소비·DONE 판정은 `design_approved`, `commit_allowed`, `phases`, `consumed_approvals`, git 상태의 조합에서 파생된다. `phase`를 자동 상태 머신 필드로 재구현하면 현재 동작과 달라진다([07](07-business-rules-and-state-machines.md) §4).
...
- **`phases[].approved`**: phase 리뷰 승인 때 sticky `true`로 바뀌지만 실제 커밋 완료 원장은 아니다. 커밋 완료·다음 phase 판정은 `consumed_approvals[]`를 사용한다.
...
`state.json`은 tracked 파일이지만 리뷰·커밋 중 생긴 변경은 clean-tree 검사에서 scratch로 무시되고, `req:commit`의 source/evidence 커밋에 stage하면 차단된다. 이 저장소의 일부 완료 티켓은 별도 수동 chore 커밋으로 최종 state를 보존했지만(예: `REQ-2026-018`), 이를 수행하는 일반 CLI 단계는 없다.

따라서 다음을 현재 보장으로 주장할 수 없다.

- 리뷰 직후 다른 clone/worktree에서 같은 `req:next` 결과가 나온다.
- 로컬 `state.json`이 사라지면 아카이브·manifest·git에서 자동 재구축된다.
- 최종 state 수동 커밋을 누락해도 티켓의 논리 완료 상태가 원격에 남는다.

감사 증거 자체는 git에 남지만 **실행 상태 뷰의 재구축**은 별도 문제다. 이 gap은 [gaps-and-decisions.md](gaps-and-decisions.md) G-09, 목표 설계는 [14](14-product-strategy-and-roadmap.md) STR-02에 정의한다.
