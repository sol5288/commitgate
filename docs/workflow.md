...
- **판정이 나온 리뷰**(승인·수정 요청처럼 결과가 정상적으로 나온 리뷰)가 `autoBudget`(기본 5)회를
  채우기 전까지는 사람 개입 없이 돕니다.
- 채운 뒤의 처리는 **`onSoftLimit`이 정합니다**:
...
- `hardCap`(기본 8)을 소진하면 그다음 회차는 **두 값 모두에서** 차단됩니다. 그때는 종료하거나
  정합한 대체 REQ를 만듭니다.
- 🔴 **두 한도는 세는 것이 다릅니다.** `autoBudget`은 **판정이 나온 리뷰**를, `hardCap`은 **실제로
  호출한 횟수**를 셉니다. 응답이 무효면 판정 수에는 안 들어가지만 호출 수에는 들어갑니다(호출은
  이미 나갔고 비용도 발생했습니다). 그래서 **"몇 번째 회차부터"라고 고정해 말할 수 없습니다.**
...
🔴 **권한 판단은 설정값이나 브랜치 이름만으로 내려지지 않습니다.** 도구는 통합 대상 SHA 의 트리에서
그 티켓의 **정책 스냅샷**을 읽습니다 — 티켓을 만들 때 `auto` 였다면 나중에 설정을 `merge` 로 바꿔도
위임 요구가 사라지지 않습니다. 브랜치 이름이 표준 형식이 아니면 **범위의 커밋 귀속**에서 대상을
찾고, 그래도 대상을 확정할 수 없으면 자동으로 진행하지 않습니다.

🔴 **브랜치 이름에서 통합 대상을 확정할 수 없을 때만 예외가 있습니다.** 그 경우 자동 통합은 하지
않지만, **대화형 최종 확인**(`[y/N]` 기본 No)으로 사람이 직접 통합할 수 있습니다. 위임이 없으므로
그 경로는 **로컬 병합까지만**이고 push 하지 않습니다.

🔴 **다른 거부는 사람 확인으로도 열리지 않습니다** — 위임 부재·만료·철회·이미 소비 · trunk 이동 ·
...
상한에 닿으면 **왜 수렴하지 않았는지**와 **지금 이 상태에서 성공하는 명령**이 함께 나옵니다.
...
- 🔴 **조언이지 증거가 아닙니다.** 승인·커밋·병합 어느 판정도 이 문구를 읽지 않습니다.
- 🔴 **새 리뷰를 부르지 않습니다.** 이미 있는 라운드 기록을 읽어 정리할 뿐입니다.
- 🔴 **`hardCap` 을 올리라는 선택지는 없습니다.** 목록에 있으면 그게 기본 답이 되기 때문입니다.
- 반복해서 걸린 축이 없으면 "매 라운드 지적이 달랐다"고 말합니다 — 한 곳이 안 풀린 것이 아니라
  **범위가 넓다**는 신호입니다. 라운드 기록이 아예 없으면 **분해안을 내지 않습니다**(관측이 없으면
  권고도 없습니다).
...
뒤쪽에서 죽으면 워킹 트리에 증거 파일이 더러운 채 남고, `req:next` 는 이렇게 안내합니다.

```sh
npx commitgate req:commit <REQ> --finalize --run
```

이 명령은 **어느 지점에서 죽었든** 남은 만큼만 이어서 합니다. 다시 실행해도 안전합니다(멱등).

- 소스 커밋만 되고 증거가 하나도 안 된 상태 → 아카이브·매니페스트부터
- 증거는 커밋됐고 소비가 안 된 상태 → 소비·checkpoint 만
- 소비까지 됐고 `state.json` 만 미커밋 → checkpoint 만
- 이미 다 끝난 상태 → 아무 일도 하지 않고 성공

### 무엇을 통과시키고 무엇을 막는가

이 복구는 **`D10`(워킹 트리 클린) 을 끄지 않습니다.** 승인 시점에 못 박아 둔 아카이브 목록
(경로 + SHA-256)과 **바이트 단위로 일치할 때만**, 정확히 그 파일들만 통과시킵니다.

| 상황 | 결과 |
|---|---|
| 아카이브 내용이 승인 이후 바뀜 | 🔴 거부 |
| 목록에 없는 아카이브가 섞임 | 🔴 거부 |
| 소스 파일·다른 티켓 파일이 더러움 | 🔴 거부 |
| 소스 커밋의 내용이 승인된 것과 다름 | 🔴 거부 |
| 커밋된 매니페스트와 `state` 의 목록이 다름 | 🔴 거부 |

거부되면 **왜인지와 다음에 할 일**을 함께 알려 줍니다. 🔴 원장·`state.json`·`approvals.jsonl` 을
손으로 고쳐서 푸는 길은 없습니다 — 그렇게 푼 것은 증거가 아닙니다.

🔴 **이 REQ 이전에 승인받은 티켓에는 그 목록이 없습니다.** 근거가 없으므로 복구를 열지 않고, 재리뷰로
새 승인을 받아야 합니다.
