...
두 명령은 **사람의 것**입니다. 에이전트는 실행하지 않고 **사용자에게 실행을 요청**합니다.
...
| `npm run req:confirm -- <id> --scope <s> --method "..." --run` | **사람의 판단을 기록**하는 명령입니다. 시각을 실제 시계에서 읽으므로 손기록으로 지어낼 수 없고, `--method`에 적는 "무엇을 근거로 승인했는지"가 감사 기록의 내용입니다. |
