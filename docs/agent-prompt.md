...
아래 명령은 **사람의 것**입니다. 에이전트는 실행하지 않고 **사용자에게 실행을 요청**합니다.

| 명령 | 왜 사람인가 |
|---|---|
| `npx commitgate setup` | **대화형 전용**입니다. 파이프·CI·에이전트 세션에서는 질문 없이 즉시 종료하므로 에이전트가 실행하면 실패만 남습니다. 리뷰 모델·추론강도·멈춤 지점·**리뷰 예산 정책**(`reviewBudget.onSoftLimit`)을 고르고 codex 로그인까지 마칩니다. |

🔴 `req:confirm`은 이 표에 **없습니다**. 그것은 사람 전용 명령이 아니라 **통제점**입니다 — 에이전트가
승인 문장을 그대로 받은 뒤 실행합니다(`req:next`가 그 명령을 출력합니다). 시각을 실제 시계에서 읽는 것은
손기록으로 지어내는 표면을 없애기 위한 것이지 실행 주체를 사람으로 못 박기 위한 것이 아닙니다.
판단은 사람이 하고(`--method`에 적는 승인 근거), 실행은 에이전트가 합니다.

신규 설치에서 setup을 마치지 않으면 `req:new`를 비롯한 워크플로 명령이 **막힙니다**. 그 상태를 만나면 에이전트는 우회를 시도하지 말고 사용자에게 setup 실행을 요청해야 합니다.
...
Matt Pocock의 공개 skills(MIT)를 CommitGate의 권한 경계에 맞게 적응한 4종과, 그 방법들을 REQ 설계·계획·구현 품질로 묶는
CommitGate 원저작 우산 스킬(`commitgate-quality`)을 더해 **5종**을 패키지에 함께 번들합니다.
...
| `commitgate-quality` | 설계·계획(`00/01/02`) 작성·수정 · AGENT 구현·버그 진단 — 정본(SSOT) 참조·조합 검증·Test-First·증거 기반 검증. **방법이지 강제 아님** |

**자동 발견 · 모델 판단 호출.** harness가 **설치된** 스킬을 자동으로 **발견**하지만, 쓸지 **판단하는 건 모델**입니다 —
확률적이며 항상 뜬다고 기대하면 안 됩니다. Claude Code에서는 설치된 스킬을 `/commitgate-<이름>`으로 **직접 호출**할 수도 있습니다.
...
- **`--no-agent-entrypoints`**: `.claude/` 계층 전체를 건너뜁니다(companion 5종 포함).
