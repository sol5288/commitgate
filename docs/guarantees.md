...
- 열린 `(review_kind, phase_id)` review series별로 재리뷰 시도를 계수합니다. 기본값은 `{ autoBudget: 5, hardCap: 8, onSoftLimit: "ask" }`입니다.
  - **판정이 나온 리뷰**가 `autoBudget`(기본 5)회를 채우기 전까지는 자동으로 진행합니다.
  - 채운 뒤는 **설정에 따라 다릅니다** — `onSoftLimit: "ask"`(기본)면 회차마다 그 series·회차에 결속된 사람 예외 기록이 있어야 진행하고, `"auto"`면 사람 승인 없이 진행하며 원장에 정책 근거를 남깁니다.
  - 🔴 **어느 값에서도** `hardCap`(기본 8) **호출**을 소진하면 그 다음 시도는 예외가 있어도 차단됩니다 — 이것이 무한 재리뷰 루프를 막는 백스톱입니다.
  - 🔴 두 한도는 **세는 기준이 다릅니다**: `autoBudget`은 **판정이 나온 리뷰**만, `hardCap`은 **실제로 나간 호출**을 셉니다(무효 응답은 앞쪽에 안 들어가고 뒤쪽에만 들어갑니다).
  - 승인되면 그 series가 닫히고, 미수렴 series를 사람이 `human-resolution`으로 종료하면 같은 키의 자동 재개가 막힙니다.
