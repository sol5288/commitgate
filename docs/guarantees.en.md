...
- Re-review attempts are counted per open `(review_kind, phase_id)` review series. The defaults are `{ autoBudget: 5, hardCap: 8, onSoftLimit: "ask" }`.
  - **Reviews that produced a verdict** run automatically until they reach `autoBudget` (default 5).
  - What happens after that **depends on configuration**: with `onSoftLimit: "ask"` (default) each round requires a human exception record bound to that series and round; with `"auto"` it proceeds without human approval and records the policy grounds in the ledger.
  - 🔴 **Under either value**, once `hardCap` (default 8) **calls** are spent the next attempt is blocked even with an exception — this is the backstop that prevents infinite re-review loops.
  - 🔴 The two limits **count different things**: `autoBudget` counts **reviews that produced a verdict**, `hardCap` counts **calls actually made** (an invalid response is excluded from the former but included in the latter).
  - An approval closes the series; if a human terminates an unconverged series with a `human-resolution`, automatic resumption for that key is stopped.
