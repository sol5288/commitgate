...
describe('[REQ-2026-140] 구조 가드 — merge 리터럴 비교가 술어 밖에 없다', () => {
  ...
  it("🔴 scripts/req·bin 어디에도 `=== 'merge'` / `!== 'merge'` 가 없다(정본 제외)", () => {
    const offenders: string[] = []
    for (const rel of [...tsFiles(join('scripts', 'req')), ...tsFiles('bin')]) {
      if (rel === SSOT) continue
      const body = readFileSync(join(ROOT, rel), 'utf8')
      body.split('\n').forEach((line, i) => {
        if (/[!=]==\s*'merge'/.test(line)) offenders.push(`${rel}:${i + 1}: ${line.trim()}`)
      })
    }
 ...
 *
 * 🔴 이 테스트는 **나중에 뒤집힌다**(노출 phase 에서 "받아들인다"로). 그것이 의도다:
 *    지금 통과한다는 것은 "중간 커밋의 사용자는 이 값을 고를 수 없고, 그래서 문서가 여전히 참"이라는
 *    사실의 오라클이다. 노출 없이 문서만 앞당기면 아직 없는 동작을 설명하게 되고,
 *    노출만 앞당기면 문서가 거짓이 된다 — 둘 다 같은 커밋이어야 한다.
 */
describe('[REQ-2026-140 DEC-9] auto 는 아직 사용자에게 노출되지 않았다', () => {
  it('스키마 enum 에 auto 가 없다', () => {
    const schema = JSON.parse(readFileSync(join(ROOT, 'workflow', 'req.config.schema.json'), 'utf8')) as {
      properties: { stopGate: { enum: string[] } }
    }
    expect(schema.properties.stopGate.enum).toEqual(['phase', 'req', 'merge'])
  })

  it('🔴 req.config.json 에 auto 를 써 넣으면 로드가 거부한다(손편집 경로도 막힌다)', () => {
  ...
  it('대조군 — merge 는 같은 경로로 로드된다(위 거부가 stopGate 때문임을 보인다)', () => {
    const root = mkdtempSync(join(tmpdir(), 'cg-auto-expose-ok-'))
    try {
      writeFileSync(join(root, 'req.config.json'), JSON.stringify({ stopGate: 'merge' }))
      const cfg = loadConfig({ root })
      expect(cfg.stopGate satisfies StopGate).toBe('merge')
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })
